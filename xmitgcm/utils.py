...
def read_mds(fname, iternum=None, use_mmap=True, force_dict=True, endian='>',
             shape=None, dtype=None, dask_delayed=True, llc=False, aste=False,
    ...
    if fldlist is None:
        fldlist = [fname]
    metadata = {'filename':datafile, 'vars':fldlist, 'dtype':dtype,'endian':endian}

    # for backward compatibility but there might be another option
    if len(shape)==4:
        nt, nz, ny, nx = shape
        dim_vars = [('nz','ny','nx')]
    elif len(shape)==3:
    ...
    metadata.update({'dims_vars': dim_vars, 'nt':nt, 'nz':nz, 'ny':ny, 'nx':nx})

    if llc:
        metadata.update({'has_faces':True,'ny_facets':[3*nx,3*nx,nx,3*nx,3*nx],
        ...
        metadata.update({'has_faces':False})

    if aste:
        metadata.update({'has_faces':True,#'nt':1, #'nz':50, #'ny':1350, 'nx':270,
        ...
        if nx == 90:
            metadata.update({'ny':1350, 'nx':270})
    print('Attempting to read', fldlist, 'nx=', nx, 'ny=', ny)
    # the way read_mds is constructed limits to file containing one variable
    d = read_all_variables(fldlist, metadata, use_mmap=use_mmap)
...
def read_all_variables(variable_list, metadata, use_mmap=True):
    ''' return a dictionay of dask array '''
    out = []
...
def read_generic_data(variable, metadata, use_mmap=True):
    ...
    if (metadata['nx'] == 1) and (metadata['ny'] == 1) and (len(metadata['vars']) == 1):
        # profile
        data_raw = read_raw_data(metadata['filename'], metadata['dtype'], (metadata['nz'],), use_mmap=use_mmap, offset=0,
                  order='C', partial_read=False)
        ...
        chunks = (metadata['nt'], 1, 1,
                  metadata['nx'], metadata['nx'])
        print('shape of resulting array', shape)
        ...
        if metadata['has_faces']:
            ...
            shape = (metadata['nt'], metadata['nz'], len(metadata['face_facets']),
                     metadata['nx'], metadata['nx'])
            name = 'llc-' + tokenize(metadata['filename'])  # unique identifier
            ...
            data = dsa.Array(dsk, name, chunks, dtype=metadata['dtype'], shape=shape)
        else:
            ...
            name = 'reg-' + tokenize(metadata['filename'])  # unique identifier
...
def _read_xy_chunk(variable,metadata,rec=0,lev=0,face=0,use_mmap=False):
    ...
    if 'nx' in dims : nx=metadata['nx']
    if 'ny' in dims : ny=metadata['ny']
    if 'nz' in dims : nz=metadata['nz']
    if 'nt' in dims : nt=metadata['nt']
    ...
    if metadata['has_faces']:
        ...
        #print(facet_origin,offset_facets,shape)
    else:
    ...
    #print(offset,partial_read,shape)
    ...
    if 'pad_before_y' in metadata:
        ...
        pad_before = dsa.from_array(np.zeros((nypad_before,nx)),chunks=(nypad_before,nx))
        ddata_padded_before = dsa.concatenate((pad_before,ddata_interior), axis=0)
    else:
    ...
    if 'pad_after_y' in metadata:
        ...
        pad_after = dsa.from_array(np.zeros((nypad_after,nx)),chunks=(nypad_after,nx))
        ddata_padded_after = dsa.concatenate((ddata_padded_before,pad_after), axis=0)
    else:
